...
def vectorize_data(train_data, test_data):
    ...
    joblib.dump(v, "DicVecFull.joblib")
...
def logistic(train_data, train_labels):
    ...
    # saving the model to disk
    filename = 'logistic_regression_model_FULL.joblib'
...
if __name__ == '__main__':
    ...
    labs = ["ABBR","CAP","LOC","POS","POS+1","POS-1","PREF","SUFF","WORD","WORD+1","WORD-1","TAG","TAG+1","TAG-1"]
    features_set = set(labs)
    print('Running Logistic Regression classification algorithm...')

    assert 'WORD' in features_set, 'WORD is a mandatory feature'
    assert features_set.issubset({"ABBR","CAP","LOC","POS","POS+1","POS-1","PREF","SUFF","WORD","WORD+1","WORD-1","TAG","TAG+1","TAG-1"}), 'There is an invalid feature name'
    ...
    #classes = ['-','AQ', 'AQB', 'AQL', 'AMT', 'PURCH', 'SELL', 'STAT']
    classes = ['-', 'B-AQ', 'I-AQ', 'B-AQB', 'I-AQB', 'B-AQL', 'I-AQL', 'B-AMT', 'I-AMT', 'B-PURCH', 'I-PURCH', 'B-SELL', 'I-SELL', 'B-STAT', 'I-STAT']
